...
* 17 - **gw_stat** - Genome wide phasing statistic between 0.5 and 1. This is the relative support for the given GW phase. A value of 0.5 indicates equal support for the two genome wide phasing configurations. In these cases the phase from the input VCF will be used.

##*out_prefix*.vcf.gz

Output of the input VCF file gzipped and containing the following added fields in the appropriate sample column:
* **GT** - phASER Genome Wide Genotype - Genome wide phasing for the alleles. The method for determing the genome wide phase is specified by --gw_phase_method. This will only be outputted (overwriting the original input phase) if --gw_phase_vcf = 1.
* **PGPS** - Genome wide phasing statistic between 0.5 and 1. This is the relative support for the given GW phase. A value of 0.5 indicates equal support for the two genome wide phasing configurations. In these cases the phase from the input VCF will be used.
